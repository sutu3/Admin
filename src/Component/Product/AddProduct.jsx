      ...
      <div className="w-full h-[200px] flex flex-row justify-around ">
        ...
        <div>
          ...
          <Link to={'/Product/AddnewProduct'}>
           <Button aria-label="Action Order Delivery" radius="full" className={`shadow-lg bg-gradient-to-tr from-pink-500 to-yellow-500 text-white`}>
      + Add New Product
    </Button></Link>
          </div>
        </div>
        ...
        <div className="h-[450px] -translate-y-10 w-[70%]">
            ...
            />
              ...
              <div className="w-full flex flex-row justify-between">
                <Button aria-label="Action Order Delivery" endContent={<FontAwesomeIcon icon={faReplyAll} />} className="border-2 border-[#5eb2f6] bg-[#FFFFFF] font-mono hover:border-white text-[#1A202C] text-sm hover:text-white  hover:bg-[#88c1ff]">Select All</Button>
                <Button aria-label="Action Order Delivery" startContent={list.length!=0?OrderPrepare[0].purchaseorderitem.reduce(
        (acc, el) => acc + el.purchase_price * el.quantity,
        0
      ):0} endContent={<FontAwesomeIcon icon={faTruck} />} className="hover:bg-[#63E6BE] text-[#63E6BE]  hover:text-white font-mono w-40" onPress={onOpen}>confirm</Button>
                <Button aria-label="Action Order Delivery" endContent={<FontAwesomeIcon icon={faDeleteLeft} />}className="border-2 border-[#f6615e] bg-[#FFFFFF] font-mono hover:border-white text-[#1A202C] text-sm hover:text-white hover:bg-[#ff8888]">Delete</Button>
              </div>
      ...
      >
        <ModalContent className="bg-white border-[2px] rounded-xl border-slate-500">
          <ModalHeader className="flex flex-col gap-1">
            Do you want to Confirm Purchase Order {`${OrderPrepare.purchase_orders_id}`}
          </ModalHeader>
          <ModalBody>
            <div>
              Click the Button And your Order will Delivery
            </div>
          </ModalBody>
          <ModalFooter>
            <Button
