...
const Index = () => {
    ...
    const [index1, setindex] = useState(data.findIndex((el)=>el.link==location.pathname));
  return (
    ...
    <div className="flex flex-row w-full font-bold justify-center gap-5 pb-2 border-b-2 border-blue-300">
      ...
      {data.map((el, index) => (
        ...
        >
          ...
          >
            <div
              className={`bg-red-300 w-10 h-10 p-1 rounded-md ${
                index === index1 ? "bg-[#646cff] shadow-md" : "bg-transparent"
              } transition duration-300 justify-center items-center flex ease-out`}
            >
            {el.flat==true?<Badge content={el.message} color="primary" className="bg-blue-300">
        <FontAwesomeIcon
                size="xl"
